...
basic_sigma_vals = logspace(-1, .71 ,10); 
N_values = 10^5;
...
%%
if to_save
    folder_name = ['EM_iterations_',datestr(now,'mmmm_dd_yy_HHMM')];
    mkdir(folder_name)
    cd(folder_name)
end
...
total_EM_iter    = zeros(length(basic_sigma_vals),length(N_values));
...
for n_val = 1:length(N_values)
    ...
    EM_iter    = zeros(length(basic_sigma_vals),max_repeated_trials);
    ...
    for trialNum = 1:max_repeated_trials      
        ...
        for noise_level = 1:length(sigma_vals)
            ...
            % Applying EM
            tic
            [x_est_em, p_est_em, likelihood, iter] = EM(yj, sigma_vals(noise_level), x_init, p_init, max_iter, tol);
            time_EM(noise_level, trialNum) = toc;
            
            % the relative error: EM
            EM_iter(noise_level, trialNum) = iter;
        end
    ...
    total_EM_iter(:,n_val)  = mean(EM_iter,2);   
    ...
    ylabel('Time (sec.)')
%     leg1 = legend('LS MoM','EM','location','best'); 
%     set(leg1,'Interpreter','latex');
%     set(leg1,'FontSize',20);
    set(gca,'FontSize',18)
 %   title(['Number of elements: ', num2str(N)]);
    if to_save
        name_it = ['EM_time_',num2str(n_val)];
    ...
    figure;   % iterations
    set(0,'defaulttextInterpreter','latex')
    semilogx(total_snr_level(:,n_val), total_EM_iter(:,n_val),':k','LineWidth',3.3);
    hold on
    semilogx(total_snr_level(:,n_val), max_iter*ones(size(total_snr_level(:,n_val))),'--r','LineWidth',1.5);
    xlabel('SNR')
    ep = 0.01;
    xlim([min(total_snr_level(:,n_val))-ep,max(total_snr_level(:,n_val))+ep])
    ylabel('Iterations')
    leg1 = legend('EM','Maximum iterations','location','NorthEast'); %best');
    set(leg1,'Interpreter','latex');
    set(leg1,'FontSize',20);
    set(gca,'FontSize',18)
   % title(['Number of elements: ', num2str(N)]);
    if to_save
        name_it = ['EM_iters_N_',num2str(n_val)];
        saveas(gcf, name_it ,'fig');
        saveas(gcf, name_it,'jpg');
        print('-depsc2',name_it);
        print('-depsc2',name_it);
